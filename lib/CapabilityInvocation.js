...
module.exports = class CapabilityInvocation extends ControllerProofPurpose {
  ...
  async validate(proof, {verificationMethod, documentLoader, expansionMap}) {
    try {
      ...
      timers.fetchInSecurityContext = timer.elapsed();
      timer = _getTimer();
      ...
      timers.verifyCapabilityChain = timer.elapsed();
      timer = _getTimer();
      ...
      timers.isInvoker = timer.elapsed();
      timer = _getTimer();
      const validateResult = await super.validate(proof, {
        documentLoader, verificationMethod, expansionMap});
      timers.superValidate = timer.elapsed();
      console.log('OCAPLD_VERIFY', JSON.stringify(timers, null, 2));
